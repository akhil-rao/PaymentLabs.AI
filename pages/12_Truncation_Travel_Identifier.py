...
# Page Title
st.set_page_config(page_title="13_Truncation_Travel_Identifier")
...
def extract_fields_pacs008(xml_content):
    try:
        ...
        return {
            "UETR": root.find('.//ns:UETR', ns).text if root.find('.//ns:UETR', ns) is not None else None,
...
if uploaded_mt103 and uploaded_pacs008:
    ...
    if mt_uetr and pacs_fields.get("UETR") == mt_uetr:
        ...
        # Extract MT fields
        mt_debtor = re.search(r":50[KF]:.*?\n(.+)", mt_text)
        mt_creditor = re.search(r":59:.*?\n(.+)", mt_text)
